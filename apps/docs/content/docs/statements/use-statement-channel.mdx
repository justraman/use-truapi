...
empty, and `write` resolves `false`. Once ready, `write` resolves `true` when
the host accepted the statement and rejects with a `StatementStoreError`
(too large, not connected, allowance missing) when it did not, so catch the
promise or surface the reason.
